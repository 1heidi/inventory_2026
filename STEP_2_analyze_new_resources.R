...
## Purpose: Analyze metadata for biodata resources identified in publication years 2024 and 2025 as done for last update.
...
## Input file(s): predictions_final_2026-08-07.csv, predictions_final_2024-07-12.csv; scripts based on those written by Ken Schackart at https://github.com/globalbiodata/inventory_2022/blob/main/analysis/location_information.R
## Output file(s): new_biodata_resources_2026_unaugmented.csv, new_biodata_resources_2026_augmented.csv, auth_countries_new_resources_2026.jpeg, url_countries_new_resources_2026.jpeg, 
...
## Changes in the IDs (e.g., for those with new publications in 2024/25), URL status, and metadata retrieved from Europe PMC (e.g., citation counts) prevents filtering by an anti_join. Additionally, filtering by just publication date returns resources from prior inventory which have had another paper published in 2024 or 2025 since publication_date lists the newest. So, to find those new to this inventory update, select just best_name, ID, and publication date to match on best_name, then double check dates.

i <- read.csv("predictions_final_2024-07-12.csv")
ui <- read.csv("predictions_final_2026-08-07.csv")
...
## Membrane Protein complex binding Affinity Database not 
...
write.csv(new, "new_biodata_resources_2026_unaugmented.csv", row.names = FALSE)

##==============================================================##
######### PART 2: Augment Country from Author Affiliations ####### 
##==============================================================##

## Missing Affiliation Countries: ## rows empty for author affiliation_countries (~##%); attempt to extract additional countries from affiliation using another package
...
mac$affiliation_countries[mac$affiliation_countries == "UK"] <- "United Kingdom"
mac$affiliation_countries[mac$affiliation_countries == "Republic of Korea"] <- "Korea"
mac$affiliation_countries[mac$affiliation_countries == "South Korea"] <- "Korea"
mac$affiliation_countries[mac$affiliation_countries == "Turki"] <- "Turkiye"
...
## check
c <- as.data.frame(unique(new_c$aug_affiliation_countries))
